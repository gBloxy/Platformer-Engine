...
class Mouse(Element):
    def __init__(self, win_size, display_size):
        super().__init__()
        self.pos = (0, 0)
        self.real_pos = (0, 0)
        ...
        self.scale_factor = (win_size[0] / display_size[0], win_size[1] / display_size[1])
...
class Input(Element):
    def __init__(self, win_size=None, display_size=None):
        if win_size and display_size:
            self.mouse = Mouse(win_size, display_size)
        else:
            super().__init__()
            self.mouse = Mouse(self.g.window.size, self.g.size)
        self.keys_pressed = []
        self.keys_pressing = []
        self.keys_released = []
        self.quit = False
